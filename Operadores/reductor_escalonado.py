...
class ReductorEscalonado:
    ...
    def __init__(self, eps: float = 1e-12, pivot_callback: Optional[Callable[[int, int], None]] = None):
        self._eps = eps
        self._pivot_callback = pivot_callback # Guardar el callback
    ...
    def a_forma_escalonada_reducida(
        ...
        for c in range(num_variables):  # solo columnas de variables (excluye término independiente)
            ...
            if self._pivot_callback:
               self._pivot_callback(fila_piv, c)
