...
from UI.ViewModels.combinacion_lineal_vm import CombinacionLinealViewModel, CombinacionLinealResultVM
from UI.ViewModels.ecuacion_vectorial_vm import EcuacionVectorialViewModel, EcuacionVectorialResultVM
from UI.ViewModels.ecuacion_matricial_vm import EcuacionMatricialViewModel, EcuacionMatricialResultVM
...
class MatrixCalculatorWindow(QMainWindow):
    ...
    def __init__(self) -> None:
        ...
        # ViewModel para combinación lineal
        self.combinacion_vm = CombinacionLinealViewModel()
        # ViewModel para ecuaciones vectoriales
        self.ecuacion_vectorial_vm = EcuacionVectorialViewModel()
        # ViewModel para ecuaciones matriciales
        self.ecuacion_matricial_vm = EcuacionMatricialViewModel()
        ...
        self.combinacion_lineal_page = self._create_combinacion_lineal_page()  
        self.ecuacion_vectorial_page = self._create_ecuacion_vectorial_page()  
        self.ecuacion_matricial_page = self._create_ecuacion_matricial_page()

        # Añadir páginas al stack
        self.stack.addWidget(self.calculator_page)
        self.stack.addWidget(self.home_page)
        self.stack.addWidget(self.vector_props_page)
        self.stack.addWidget(self.combinacion_lineal_page)  
        self.stack.addWidget(self.ecuacion_vectorial_page)  
        self.stack.addWidget(self.ecuacion_matricial_page) 
    ...
    def _create_nav_panel(self) -> QWidget:
        ...
        self.btn_combinacion_lineal = QPushButton("Combinación Lineal")
        self.btn_combinacion_lineal.setObjectName("navButton")
        self.btn_combinacion_lineal.setCursor(Qt.PointingHandCursor)
        self.btn_combinacion_lineal.setCheckable(True)
        self.btn_combinacion_lineal.setAutoExclusive(True)
        self.btn_combinacion_lineal.clicked.connect(lambda: self.stack.setCurrentIndex(3))  
        # Página 3
        nav_layout.addWidget(self.btn_combinacion_lineal)

        self.btn_ecuacion_vectorial = QPushButton("Ecuación Vectorial")
        self.btn_ecuacion_vectorial.setObjectName("navButton")
        self.btn_ecuacion_vectorial.setCursor(Qt.PointingHandCursor)
        self.btn_ecuacion_vectorial.setCheckable(True)
        self.btn_ecuacion_vectorial.setAutoExclusive(True)
        self.btn_ecuacion_vectorial.clicked.connect(lambda: self.stack.setCurrentIndex(4)) 
         # Página 4
        nav_layout.addWidget(self.btn_ecuacion_vectorial)

        self.btn_ecuacion_matricial = QPushButton("Ecuación Matricial")
        self.btn_ecuacion_matricial.setObjectName("navButton")
        self.btn_ecuacion_matricial.setCursor(Qt.PointingHandCursor)
        self.btn_ecuacion_matricial.setCheckable(True)
        self.btn_ecuacion_matricial.setAutoExclusive(True)
        self.btn_ecuacion_matricial.clicked.connect(lambda: self.stack.setCurrentIndex(5))  
        # Página 5
        nav_layout.addWidget(self.btn_ecuacion_matricial)
    ...
    def _create_combinacion_lineal_page(self) -> QWidget:
        """Página para verificar combinación lineal de vectores"""
        page = QWidget()
        layout = QVBoxLayout()
        page.setLayout(layout)
        ...
        # Explicación
        explicacion = QLabel(
            "Verifica si un vector objetivo puede expresarse como combinación lineal "
            "de un conjunto de vectores. Ingresa los vectores separados por comas o espacios.\n"
            "Ejemplo: (1,2,3) o 1 2 3"
        )
        explicacion.setWordWrap(True)
        layout.addWidget(explicacion)

        # Controles para cantidad de vectores y dimensión
        config_layout = QHBoxLayout()
        config_layout.addWidget(QLabel("Número de vectores:"))
        self.combinacion_num_vectores = QSpinBox()
        self.combinacion_num_vectores.setMinimum(1)
        self.combinacion_num_vectores.setMaximum(10)
        self.combinacion_num_vectores.setValue(2)
        config_layout.addWidget(self.combinacion_num_vectores)

        config_layout.addWidget(QLabel("Dimensión de vectores:"))
        self.combinacion_dimension = QSpinBox()
        self.combinacion_dimension.setMinimum(1)
        self.combinacion_dimension.setMaximum(10)
        self.combinacion_dimension.setValue(2)
        config_layout.addWidget(self.combinacion_dimension)

        layout.addLayout(config_layout)

        # Campos para ingresar vectores
        self.combinacion_vector_inputs = []
        self.vectores_layout = QVBoxLayout()
        layout.addLayout(self.vectores_layout)

        # Botón para generar campos de vectores
        btn_generar = QPushButton("Generar campos para vectores")
        btn_generar.clicked.connect(self._generar_campos_combinacion)
        layout.addWidget(btn_generar)

        # Vector objetivo
        objetivo_layout = QHBoxLayout()
        objetivo_layout.addWidget(QLabel("Vector objetivo:"))
        self.combinacion_objetivo_input = QLineEdit()
        self.combinacion_objetivo_input.setPlaceholderText("Ej: 3,3")
        objetivo_layout.addWidget(self.combinacion_objetivo_input)
        layout.addLayout(objetivo_layout)

        # Botones de acción
        btn_layout = QHBoxLayout()
        btn_verificar = QPushButton("Verificar Combinación Lineal")
        btn_verificar.clicked.connect(self._on_verificar_combinacion)
        btn_limpiar = QPushButton("Limpiar")
        btn_limpiar.clicked.connect(self._limpiar_combinacion)
        btn_layout.addWidget(btn_verificar)
        btn_layout.addWidget(btn_limpiar)
        layout.addLayout(btn_layout)

        # Área de resultados
        self.combinacion_result_output = QTextEdit()
        self.combinacion_result_output.setReadOnly(True)
        self.combinacion_result_output.setMinimumHeight(300)
        layout.addWidget(self.combinacion_result_output)

        # Generar campos iniciales
        self._generar_campos_combinacion()

        return page

    def _create_ecuacion_vectorial_page(self) -> QWidget:
        """Página para resolver ecuaciones vectoriales"""
        page = QWidget()
        layout = QVBoxLayout()
        page.setLayout(layout)

        # Título
        title = QLabel("Ecuación Vectorial")
        font = QFont()
        font.setPointSize(16)
        font.setBold(True)
        title.setFont(font)
        ...
        # Explicación
        explicacion = QLabel(
            "Resuelve ecuaciones del tipo c₁v₁ + c₂v₂ + ... + cₙvₙ = b\n"
            "Donde cᵢ son escalares desconocidos y vᵢ, b son vectores conocidos."
        )
        explicacion.setWordWrap(True)
        layout.addWidget(explicacion)
        ...
        config_layout = QHBoxLayout()
        config_layout.addWidget(QLabel("Número de vectores:"))
        self.ecuacion_vectorial_num_vectores = QSpinBox()
        self.ecuacion_vectorial_num_vectores.setMinimum(1)
        self.ecuacion_vectorial_num_vectores.setMaximum(10)
        self.ecuacion_vectorial_num_vectores.setValue(2)
        config_layout.addWidget(self.ecuacion_vectorial_num_vectores)

        config_layout.addWidget(QLabel("Dimensión de vectores:"))
        self.ecuacion_vectorial_dimension = QSpinBox()
        self.ecuacion_vectorial_dimension.setMinimum(1)
        self.ecuacion_vectorial_dimension.setMaximum(10)
        self.ecuacion_vectorial_dimension.setValue(2)
        config_layout.addWidget(self.ecuacion_vectorial_dimension)

        layout.addLayout(config_layout)

        # Campos para ingresar vectores
        self.ecuacion_vectorial_inputs = []
        self.ecuacion_vectorial_vectores_layout = QVBoxLayout()
        layout.addLayout(self.ecuacion_vectorial_vectores_layout)

        # Botón para generar campos de vectores
        btn_generar = QPushButton("Generar campos para vectores")
        btn_generar.clicked.connect(self._generar_campos_ecuacion_vectorial)
        layout.addWidget(btn_generar)

        # Vector b
        b_layout = QHBoxLayout()
        b_layout.addWidget(QLabel("Vector b:"))
        self.ecuacion_vectorial_b_input = QLineEdit()
        self.ecuacion_vectorial_b_input.setPlaceholderText("Ej: 4,5")
        b_layout.addWidget(self.ecuacion_vectorial_b_input)
        layout.addLayout(b_layout)

        # Botones de acción
        btn_layout = QHBoxLayout()
        btn_resolver = QPushButton("Resolver Ecuación Vectorial")
        btn_resolver.clicked.connect(self._on_resolver_ecuacion_vectorial)
        btn_limpiar = QPushButton("Limpiar")
        btn_limpiar.clicked.connect(self._limpiar_ecuacion_vectorial)
        btn_layout.addWidget(btn_resolver)
        btn_layout.addWidget(btn_limpiar)
        layout.addLayout(btn_layout)

        # Área de resultados
        self.ecuacion_vectorial_result_output = QTextEdit()
        self.ecuacion_vectorial_result_output.setReadOnly(True)
        self.ecuacion_vectorial_result_output.setMinimumHeight(300)
        layout.addWidget(self.ecuacion_vectorial_result_output)

        # Generar campos iniciales
        self._generar_campos_ecuacion_vectorial()

        return page

    def _create_ecuacion_matricial_page(self) -> QWidget:
        """Página para resolver ecuaciones matriciales"""
        page = QWidget()
        layout = QVBoxLayout()
        page.setLayout(layout)

        # Título
        title = QLabel("Ecuación Matricial")
        font = QFont()
        font.setPointSize(16)
        font.setBold(True)
        title.setFont(font)
        ...
        # Explicación
        explicacion = QLabel(
            "Resuelve ecuaciones del tipo AX = B\n"
            "Donde A es una matriz conocida, X es la matriz desconocida y B puede ser un vector o matriz."
        )
        explicacion.setWordWrap(True)
        layout.addWidget(explicacion)
        ...
        a_layout.addWidget(QLabel("Matriz A (una fila por línea, elementos separados por espacios o comas):"))
        self.ecuacion_matricial_a_input = QTextEdit()
        self.ecuacion_matricial_a_input.setMaximumHeight(100)
        self.ecuacion_matricial_a_input.setPlaceholderText("Ej:\n1 2\n3 4")
        a_layout.addWidget(self.ecuacion_matricial_a_input)
        layout.addLayout(a_layout)

        # Campos para matriz B
        b_layout = QVBoxLayout()
        b_layout.addWidget(QLabel("Matriz B (una fila por línea, elementos separados por espacios o comas):"))
        self.ecuacion_matricial_b_input = QTextEdit()
        self.ecuacion_matricial_b_input.setMaximumHeight(100)
        self.ecuacion_matricial_b_input.setPlaceholderText("Ej:\n5\n6")
        b_layout.addWidget(self.ecuacion_matricial_b_input)
        layout.addLayout(b_layout)

        # Botones de acción
        btn_layout = QHBoxLayout()
        btn_resolver = QPushButton("Resolver Ecuación Matricial")
        btn_resolver.clicked.connect(self._on_resolver_ecuacion_matricial)
        btn_limpiar = QPushButton("Limpiar")
        btn_limpiar.clicked.connect(self._limpiar_ecuacion_matricial)
        btn_layout.addWidget(btn_resolver)
        btn_layout.addWidget(btn_limpiar)
        layout.addLayout(btn_layout)

        # Área de resultados
        self.ecuacion_matricial_result_output = QTextEdit()
        self.ecuacion_matricial_result_output.setReadOnly(True)
        self.ecuacion_matricial_result_output.setMinimumHeight(300)
        layout.addWidget(self.ecuacion_matricial_result_output)
    ...
    def _generar_campos_combinacion(self):
        """Genera campos para ingresar los vectores según la cantidad especificada"""
        ...
        self.combinacion_vector_inputs.clear()
        
        # Crear nuevos campos
        num_vectores = self.combinacion_num_vectores.value()
        for i in range(num_vectores):
            layout = QHBoxLayout()
            layout.addWidget(QLabel(f"Vector {i+1}:"))
            input_field = QLineEdit()
            input_field.setPlaceholderText(f"Ej: 2,-1")
            layout.addWidget(input_field)
            self.combinacion_vector_inputs.append(input_field)
            self.vectores_layout.addLayout(layout)

    def _on_verificar_combinacion(self):
        """Maneja el evento de verificar combinación lineal"""
        try:
            # Obtener vectores ingresados
            vectores = []
            for input_field in self.combinacion_vector_inputs:
                texto = input_field.text().strip()
                if texto:
                    vector = self.combinacion_vm.parse_vector(texto)
                    vectores.append(vector)
            
            # Obtener vector objetivo
            texto_objetivo = self.combinacion_objetivo_input.text().strip()
            if not texto_objetivo:
                raise ValueError("Debe ingresar un vector objetivo")
            objetivo = self.combinacion_vm.parse_vector(texto_objetivo)
            
            # Verificar combinación lineal
            resultado = self.combinacion_vm.verificar_combinacion_lineal(vectores, objetivo)
            
            # Mostrar resultados
            output_text = []
            output_text.append(f"¿Es combinación lineal? {'Sí' if resultado.es_combinacion_lineal else 'No'}")
            output_text.append("")
            output_text.append("Pasos:")
            for paso in resultado.pasos:
                output_text.append(f"• {paso}")
            
            if resultado.coeficientes:
                output_text.append("")
                output_text.append("Coeficientes encontrados:")
                for i, coef in enumerate(resultado.coeficientes, 1):
                    output_text.append(f"k{i} = {coef:.6g}")
            
            self.combinacion_result_output.setPlainText("\n".join(output_text))
            
        except Exception as e:
            QMessageBox.critical(self, "Error", str(e))

    def _limpiar_combinacion(self):
        """Limpia los campos de combinación lineal"""
        for input_field in self.combinacion_vector_inputs:
            input_field.clear()
        self.combinacion_objetivo_input.clear()
        self.combinacion_result_output.clear()

    def _generar_campos_ecuacion_vectorial(self):
        """Genera campos para ingresar los vectores según la cantidad especificada"""
        # Limpiar campos existentes
        for i in reversed(range(self.ecuacion_vectorial_vectores_layout.count())):
            widget = self.ecuacion_vectorial_vectores_layout.itemAt(i).widget()
            if widget is not None:
                widget.deleteLater()
        
        self.ecuacion_vectorial_inputs.clear()
        
        # Crear nuevos campos
        num_vectores = self.ecuacion_vectorial_num_vectores.value()
        for i in range(num_vectores):
            layout = QHBoxLayout()
            layout.addWidget(QLabel(f"Vector {i+1}:"))
            input_field = QLineEdit()
            input_field.setPlaceholderText(f"Ej: 2,-1")
            layout.addWidget(input_field)
            self.ecuacion_vectorial_inputs.append(input_field)
            self.ecuacion_vectorial_vectores_layout.addLayout(layout)

    def _on_resolver_ecuacion_vectorial(self):
        """Maneja el evento de resolver ecuación vectorial"""
        try:
            # Obtener vectores ingresados
            vectores = []
            for input_field in self.ecuacion_vectorial_inputs:
                texto = input_field.text().strip()
                if texto:
                    vector = self.ecuacion_vectorial_vm.parse_vector(texto)
                    vectores.append(vector)
            
            # Obtener vector b
            texto_b = self.ecuacion_vectorial_b_input.text().strip()
            if not texto_b:
                raise ValueError("Debe ingresar un vector b")
            b = self.ecuacion_vectorial_vm.parse_vector(texto_b)
            
            # Resolver ecuación vectorial
            resultado = self.ecuacion_vectorial_vm.resolver_ecuacion_vectorial(vectores, b)
            
            # Mostrar resultados
            output_text = []
            output_text.append(f"Estado: {resultado.estado}")
            output_text.append("")
            output_text.append("Pasos:")
            for paso in resultado.pasos:
                output_text.append(f"• {paso}")
            ...
            if resultado.coeficientes:
                ...
                output_text.append("Coeficientes encontrados:")
                for i, coef in enumerate(resultado.coeficientes, 1):
                    output_text.append(f"c{i} = {coef:.6g}")
            
            self.ecuacion_vectorial_result_output.setPlainText("\n".join(output_text))
            
        except Exception as e:
            QMessageBox.critical(self, "Error", str(e))

    def _limpiar_ecuacion_vectorial(self):
        """Limpia los campos de ecuación vectorial"""
        for input_field in self.ecuacion_vectorial_inputs:
            input_field.clear()
        self.ecuacion_vectorial_b_input.clear()
        self.ecuacion_vectorial_result_output.clear()

    def _on_resolver_ecuacion_matricial(self):
        """Maneja el evento de resolver ecuación matricial"""
        try:
            # Obtener matriz A
            texto_a = self.ecuacion_matricial_a_input.toPlainText().strip()
            if not texto_a:
                raise ValueError("Debe ingresar la matriz A")
            A = self.ecuacion_matricial_vm.parse_matriz(texto_a)
            
            # Obtener matriz B
            texto_b = self.ecuacion_matricial_b_input.toPlainText().strip()
            if not texto_b:
                raise ValueError("Debe ingresar la matriz B")
            B = self.ecuacion_matricial_vm.parse_matriz(texto_b)
            
            # Resolver ecuación matricial
            resultado = self.ecuacion_matricial_vm.resolver_ecuacion_matricial(A, B)
            ...
            if resultado.solucion:
                output_text.append("")
                output_text.append("Solución X:")
                for fila in resultado.solucion.datos:
                    output_text.append(str([f"{x:.6g}" for x in fila]))
            
            self.ecuacion_matricial_result_output.setPlainText("\n".join(output_text))
            
        except Exception as e:
            QMessageBox.critical(self, "Error", str(e))

    def _limpiar_ecuacion_matricial(self):
        """Limpia los campos de ecuación matricial"""
        self.ecuacion_matricial_a_input.clear()
        self.ecuacion_matricial_b_input.clear()
        self.ecuacion_matricial_result_output.clear()
