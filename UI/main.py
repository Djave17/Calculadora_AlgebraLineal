...
class MatrixCalculatorWindow(QMainWindow):
    ...
    def __init__(self) -> None:
        ...
        self.view_model.pivot_selected.connect(self._highlight_pivot)
    ...
    def _highlight_pivot(self, row: int, col: int) -> None:
            # Primero, limpiar cualquier resaltado anterior
            self._clear_pivot_highlight()
            # Resaltar la nueva celda del pivote
            item = self.table.item(row, col)
            if item:
                # Guardar el color original para restaurarlo después
                self._last_highlighted_item = item
                self._original_item_background = item.background()
                # Aplicar un color de fondo para el pivote
                item.setBackground(QColor(255, 255, 0, 150)) # Amarillo semi-transparente
                # Opcional: cambiar el color del texto o hacerlo negrita
                # item.setForeground(QColor(0, 0, 0))
                # font = item.font()
                # font.setBold(True)
                # item.setFont(font)
            # Opcional: Desplazarse a la celda del pivote si no está visible
            self.table.scrollToItem(item, QAbstractItemView.PositionAtCenter)
            # Pequeña pausa para que el usuario pueda ver el cambio
            # Esto es crucial para la visualización "en tiempo real"
            QApplication.processEvents() # Procesar eventos pendientes para actualizar la UI
            import time
            time.sleep(0.2) # Pausa de 200 milisegundos (ajustar según se desee)
    
    def _clear_pivot_highlight(self) -> None:
            # Restaurar el color original del último elemento resaltado
            if hasattr(self, '_last_highlighted_item') and self._last_highlighted_item:
                self._last_highlighted_item.setBackground(self._original_item_background)
                # Opcional: restaurar fuente
                # font = self._last_highlighted_item.font()
                # font.setBold(False)
                # self._last_highlighted_item.setFont(font)
                self._last_highlighted_item = None
                self._original_item_background = None
    
    def _update_table_from_list(self, matrix_data: List[List[float]]) -> None:
