...
class MatrixCalculatorWindow(QMainWindow):
    ...
    def _create_step_widget(self, step: StepVM) -> QWidget:
        ...
        Ahora destaca la celda correspondiente al pivote (pivot_row, pivot_col)
        si el StepVM contiene dicha información. El resaltado usa fondo
        amarillo suave y pone el texto en negrita para que sea fácil verlo.
        """
        widget = QGroupBox(f"Paso {step.number} – {step.description}")
        layout = QVBoxLayout()
        widget.setLayout(layout)

        # Si el StepVM incluye info del pivote, mostrar una pequeña etiqueta
        pivot_row = getattr(step, "pivot_row", None)
        pivot_col = getattr(step, "pivot_col", None)
        if pivot_row is not None and pivot_col is not None:
            # mostrar coordenadas (1-based para el usuario)
            info = QLabel(f"Pivote en fila {pivot_row + 1}, columna {pivot_col + 1}")
            info.setStyleSheet("font-size: 10pt; color: #ffd27f;")
            layout.addWidget(info)

        matrix = getattr(step, "after_matrix", None) or []
        if not matrix:
            return widget
        rows = len(matrix)
        ...
        table.setFixedHeight(26 * rows + 2)

        # Create table items and, if matches pivot, highlight
        for i in range(rows):
            for j in range(cols):
                text = f"{matrix[i][j]:.6g}"
                item = QTableWidgetItem(text)
                item.setTextAlignment(Qt.AlignCenter)
                # If pivot coordinates provided and match this cell -> highlight
                try:
                    is_pivot = (pivot_row is not None and pivot_col is not None and
                                i == int(pivot_row) and j == int(pivot_col))
                except Exception:
                    is_pivot = False
                if is_pivot:
                    # Fondo amarillo suave y texto en negrita
                    item.setBackground(QColor(255, 223, 93))  # amarillo cálido
                    f = item.font()
                    f.setBold(True)
                    item.setFont(f)
