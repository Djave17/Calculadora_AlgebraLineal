...
from UI.ViewModels.resolucion_matriz_vm import MatrixCalculatorViewModel, ResultVM, StepVM, LinearCombinationResultVM 
...
class MatrixCalculatorWindow(QMainWindow):
    ...
    # Límites para el tamaño de los vectores (para combinación lineal)
    MIN_VECTORS = 2
    MAX_VECTORS = 5
    MIN_VECTOR_DIM = 2
    MAX_VECTOR_DIM = 5
    ...
    def __init__(self) -> None:
        ...
        self.linear_combination_page = self._create_linear_combination_page()
        self.home_page = self._create_home_page()
        self.stack.addWidget(self.calculator_page)
        self.stack.addWidget(self.linear_combination_page)
    ...
    def _create_nav_panel(self) -> QWidget:
        ...
        # Nuevo botón para la combinación lineal
        self.btn_linear_combination_page = QPushButton("Combinación Lineal")
        self.btn_linear_combination_page.setObjectName("navButton")
        self.btn_linear_combination_page.setCursor(Qt.PointingHandCursor)
        self.btn_linear_combination_page.setCheckable(True)
        self.btn_linear_combination_page.setAutoExclusive(True)
        self.btn_linear_combination_page.clicked.connect(lambda: self.stack.setCurrentIndex(1)) # Índice 1 para la nueva página
        nav_layout.addWidget(self.btn_linear_combination_page)
    ...
    def _create_linear_combination_page(self) -> QWidget:
        """Crea la página para la combinación lineal de vectores."""
        page = QWidget()
        ...
        # Panel de configuración para combinación lineal
        config_panel = QGroupBox("Configuración de Combinación Lineal")
        config_layout = QVBoxLayout()
        config_panel.setLayout(config_layout)

        subtitle = QLabel(
            "Determina si un vector objetivo puede expresarse como combinación lineal de un conjunto de vectores dados."
        )
        ...
        font.setPointSize(10)
        subtitle.setFont(font)
        config_layout.addWidget(subtitle)
        grid = QGridLayout()
        config_layout.addLayout(grid)

        # Número de vectores
        num_vectors_label = QLabel("Número de vectores (k):")
        self.num_vectors_spin = QSpinBox()
        self.num_vectors_spin.setMinimum(self.MIN_VECTORS)
        self.num_vectors_spin.setMaximum(self.MAX_VECTORS)
        self.num_vectors_spin.setValue(2) # Valor inicial
        self.num_vectors_spin.valueChanged.connect(self._on_linear_combination_dimensions_changed)
        grid.addWidget(num_vectors_label, 0, 0)
        grid.addWidget(self.num_vectors_spin, 0, 1)

        # Dimensión de los vectores
        vector_dim_label = QLabel("Dimensión de vectores (n):")
        self.vector_dim_spin = QSpinBox()
        self.vector_dim_spin.setMinimum(self.MIN_VECTOR_DIM)
        self.vector_dim_spin.setMaximum(self.MAX_VECTOR_DIM)
        self.vector_dim_spin.setValue(2) # Valor inicial
        self.vector_dim_spin.valueChanged.connect(self._on_linear_combination_dimensions_changed)
        grid.addWidget(vector_dim_label, 1, 0)
        grid.addWidget(self.vector_dim_spin, 1, 1)
        config_layout.addStretch()

        # Botón para calcular
        calculate_button = QPushButton("Calcular Combinación Lineal")
        calculate_button.clicked.connect(self._on_linear_combination_clicked)
        config_layout.addWidget(calculate_button)
        main_layout.addWidget(config_panel)
        # Panel de entrada de vectores y vector objetivo
        input_panel = QGroupBox("Vectores y Vector Objetivo")
        input_layout = QVBoxLayout()
        input_panel.setLayout(input_layout)

        self.vectors_input_widgets: List[QLineEdit] = []
        self.vector_target_input_widget: QLineEdit = QLineEdit()
        self.vectors_layout = QVBoxLayout()
        input_layout.addLayout(self.vectors_layout)
        input_layout.addWidget(QLabel("Vector Objetivo (separado por espacios o comas):"))
        self.vector_target_input_widget = QLineEdit()
        self.vector_target_input_widget.setPlaceholderText("Ej: 1 2 3")
        input_layout.addWidget(self.vector_target_input_widget)

        self._update_linear_combination_input_fields() # Inicializar campos de entrada
        main_layout.addWidget(input_panel, stretch=1)

        # Panel de resultados de combinación lineal
        result_panel = QGroupBox("Resultado y Procedimiento")
        result_layout = QVBoxLayout()
        result_panel.setLayout(result_layout)

        self.lc_status_label = QLabel("Estado: ")
        self.lc_status_label.setFont(QFont("Arial", 12, QFont.Bold))
        result_layout.addWidget(self.lc_status_label)

        self.lc_solution_label = QLabel("Solución: ")
        self.lc_solution_label.setFont(QFont("Arial", 11))
        result_layout.addWidget(self.lc_solution_label)

        self.lc_procedure_title = QLabel("Procedimiento (Matriz Aumentada y Pasos Gauss-Jordan):")
        self.lc_procedure_title.setFont(QFont("Arial", 11, QFont.Bold))
        result_layout.addWidget(self.lc_procedure_title)

        self.lc_procedure_scroll_area = QScrollArea()
        self.lc_procedure_scroll_area.setWidgetResizable(True)
        self.lc_procedure_container = QWidget()
        self.lc_procedure_layout = QVBoxLayout()
        self.lc_procedure_container.setLayout(self.lc_procedure_layout)
        self.lc_procedure_scroll_area.setWidget(self.lc_procedure_container)
        result_layout.addWidget(self.lc_procedure_scroll_area)
    ...
    # ------------------------------------------------------------------
    # Manejadores de eventos para Combinación Lineal
    # ------------------------------------------------------------------
    def _on_linear_combination_dimensions_changed(self) -> None:
        """Actualiza los campos de entrada de vectores cuando cambian las dimensiones."""
        self._update_linear_combination_input_fields()
        self._clear_linear_combination_results()

    def _update_linear_combination_input_fields(self) -> None:
        """Actualiza dinámicamente los QLineEdit para los vectores y el vector objetivo."""
        k = self.num_vectors_spin.value()
        n = self.vector_dim_spin.value()

        # Limpiar campos de entrada existentes
        for widget in self.vectors_input_widgets:
            widget.deleteLater()
        self.vectors_input_widgets.clear()

        # Crear nuevos campos de entrada para los vectores
        for i in range(k):
            label = QLabel(f"Vector v{i+1} (dim {n}, separado por espacios o comas):")
            line_edit = QLineEdit()
            line_edit.setPlaceholderText(f"Ej: {' '.join(['0'] * n)}")
            self.vectors_layout.addWidget(label)
            self.vectors_layout.addWidget(line_edit)
            self.vectors_input_widgets.append(line_edit)
        
        # Actualizar placeholder del vector objetivo
        self.vector_target_input_widget.setPlaceholderText(f"Ej: {' '.join(['0'] * n)}")

    def _on_linear_combination_clicked(self) -> None:
        """Maneja el evento de clic para calcular la combinación lineal."""
        try:
            k = self.num_vectors_spin.value()
            n = self.vector_dim_spin.value()

            vectors_data: List[List[float]] = []
            for i, line_edit in enumerate(self.vectors_input_widgets):
                s = line_edit.text().strip().replace(",", " ")
                parts = [p for p in s.split() if p]
                if len(parts) != n:
                    raise ValueError(f"El vector v{i+1} debe tener exactamente {n} números.")
                vectors_data.append([float(p) for p in parts])
            
            s_target = self.vector_target_input_widget.text().strip().replace(",", " ")
            parts_target = [p for p in s_target.split() if p]
            if len(parts_target) != n:
                raise ValueError(f"El vector objetivo debe tener exactamente {n} números.")
            vector_target_data = [float(p) for p in parts_target]

            result = self.view_model.solve_linear_combination(vectors_data, vector_target_data)
            self._display_linear_combination_result(result)
    ...
    def _display_linear_combination_result(self, result: LinearCombinationResultVM) -> None:
        """Muestra los resultados de la combinación lineal en la UI."""
        self._clear_linear_combination_results()

        self.lc_status_label.setText(f"Estado: {result.status}")

        if result.status == "Combinación Lineal Única":
            coeffs_str = ", ".join([f"c{i+1} = {c:.6g}" for i, c in enumerate(result.coefficients or [])])
            self.lc_solution_label.setText(f"El vector objetivo es una combinación lineal única con coeficientes: {coeffs_str}")
        elif result.status == "Infinitas Combinaciones Lineales":
            self.lc_solution_label.setText("El vector objetivo puede expresarse como infinitas combinaciones lineales.")
            # Aquí podrías añadir la visualización paramétrica si el ViewModel la proporciona
        else: # No es Combinación Lineal
            self.lc_solution_label.setText("El vector objetivo NO puede expresarse como combinación lineal de los vectores dados.")
        
        # Mostrar el procedimiento (matriz aumentada y pasos)
        if result.augmented_matrix_before_rref:
            self.lc_procedure_layout.addWidget(QLabel("Matriz Aumentada Inicial:"))
            initial_table = self._create_matrix_table_widget(result.augmented_matrix_before_rref)
            self.lc_procedure_layout.addWidget(initial_table)
        
        if result.steps:
            self.lc_procedure_layout.addWidget(QLabel("Pasos de Gauss-Jordan:"))
            for step in result.steps:
                step_widget = self._create_step_widget(step)
                self.lc_procedure_layout.addWidget(step_widget)
        self.lc_procedure_layout.addStretch()

    def _clear_linear_combination_results(self) -> None:
        """Limpia los resultados de la combinación lineal."""
        self.lc_status_label.clear()
        self.lc_solution_label.clear()
        while self.lc_procedure_layout.count():
            child = self.lc_procedure_layout.takeAt(0)
            widget = child.widget()
            if widget is not None:
                widget.deleteLater()
    

    def _create_matrix_table_widget(self, matrix_data: List[List[float]]) -> QTableWidget:
        """Crea un QTableWidget a partir de una lista de listas de floats."""
        ...
        table.setFixedHeight(20 * rows + 2) # Ajustar altura
        table.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch)
        table.setEditTriggers(QAbstractItemView.NoEditTriggers) # Solo lectura

        for i in range(rows):
            for j in range(cols):
                item = QTableWidgetItem(f"{matrix_data[i][j]:.6g}")
                item.setTextAlignment(Qt.AlignCenter)
